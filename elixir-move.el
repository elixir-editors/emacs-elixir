...
See also `elixir-up-statement': up from current definition to next beginning of statement above. "
  (interactive)
  ...
  elixir-bol-forms-last-indent))(cdr elixir-bol-forms-last-indent))))
	 (orig (point))
         erg)
    (if indent
        (while (and (setq erg (elixir-beginning-of-statement)) (< indent (current-indentation))(not (bobp))))
...
Optional argument REPEAT, the number of loops done already, is checked for elixir-max-specpdl-size error. Avoid eternal loops due to missing string delimters etc. "
  (interactive)
  (unless (eobp)
    (let ((repeat (or (and repeat (1+ repeat)) 0))
      ...
      (cond
       ...
       ((< elixir-max-specpdl-size repeat)
        (error "elixir-end-of-statement reached loops max. If no error, customize `elixir-max-specpdl-size'"))
       ;; list
       ((nth 1 pps)
        (if (<= orig (point))
	    (progn
	...
	(when (elixir-end-of-string)
	  ...
	  (skip-chars-backward " \t\r\n\f")
	  (setq pps (parse-partial-sexp (point-min) (point)))
	  (unless (and done (not (or (nth 1 pps) (nth 8 pps))) (eolp)) (elixir-end-of-statement orig done repeat))))
